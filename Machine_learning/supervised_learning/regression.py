...
class l1_regularization():
    ...
    def __call__(self, w):
        return self.alpha * np.linalg.norm(w) #norm 默认是1范式？？？
    def grad(self,w):
...
class l1_l2_regularization():
    """Regularization for Rlastic Net Regression"""
    """L1 L2正则"""
...
class Regression(object):
    """
    Base regression model Model the Relations between a scalar dependent variable
    ...
    def initialize_weight(self,n_features):
        """Initalize weights randomly [-1/n,1/N]"""
    ...
    def fit(self,X,y):
        ...
        self.initialize_weight(n_features=X.shape[1])
...
class LinearRegression(Regression):
    ...
    def fit(self,X,y):
        ...
        if not self.gradient_descent:
            #insert constant ones for bais weights
            X = np.insert(X,0,1,axis=1)
            #Calculate weight by least squares
...
class LassoRegression(Regression):
    ...
    def predict(self,X):
        X = normalize(polynomial_features(X,degree=self.degree))
        return (LassoRegression,self).predict(X)
