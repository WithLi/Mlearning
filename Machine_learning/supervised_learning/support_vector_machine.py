...
class SuperVectorMachine(object):
    ...
    """
    def __init__(self,C=1,kernel=rbf_kernal,power=4,gamma=None,coef=4):
        self.C = C
        self.kernel = kernel
        self.power = power
        self.gamma = gamma
    ...
    def fit(self,X,y):
        ...
        """计算kernal 矩阵"""
        kernal_matrix = np.zeros((n_samples,n_samples))

        for i in range(n_samples):
            for j in  range(n_samples):
                kernal_matrix[i,j] = self.kernel(X[i],X[j])

        #Define the quadratic optimization problem
        P = cvxopt.matrix(np.outer(y,y) * kernal_matrix,tc='d')
    ...
    def predict(self,X):
        y_pred = []
        #Iterate through list of sample and make predictions
        for sample in X:
            prediction = 0
